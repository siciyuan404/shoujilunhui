...
- **录入方式**（Web 端「📒 记账」页 / Android APP 首页账本按钮）：
  - 拍照（Android 直接调相机）或选多张图 → 逐张识别图中手机（复用视觉识别 + 报价库匹配，1 图多台、多批上传均可）→ 自动带出型号/品牌/报价库参考价；识别前可填**识别提示词**引导模型；默认收货渠道「路边收」→ 核对收价/渠道/出货价/出货渠道后批量入账
  - 手动录入：输入型号自动联想报价库（带出品牌/分类/参考收价），填收价/渠道/出货价/出货渠道/状态（在库·已出）/日期
- **每台记录字段**：照片、品牌、分类、型号、收购价、渠道（收货）、出货渠道、出货价（可后填）、状态、日期、备注
- **修正**：识别错误时两端均可编辑改型号/价格/渠道/出货渠道；也可删除重录；Web 端对已入账记录可直接「重新识别」照片修正型号；Android 待入账清单每台带「🔄 重识别」（用原图+该机位置框裁剪单独重识别）；明细卡显示预计毛利（出货价−收购价）
