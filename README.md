...
- 「+ 添加型号」新增、× 删除（带确认）
- 远程（经内网穿透）打开为只读模式
- **高级细节筛选**：「⚙ 筛选」按 上市年份 / CPU品牌 / 后置摄像头(≥) / 运行内存 / 存储 / 网络制式 / 屏幕材质 / 回收价区间 组合筛选
- **机型档案（规格库）**：每个型号可维护 上市时间 / CPU品牌 / CPU型号 / 内存 / 存储 / 前后摄像头 / 屏幕尺寸 / 材质 / 刷新率 / 电池 / 快充 / 网络 / 系统，详情页展示、可编辑
- **分容分版本价格**：同一型号按容量/版本维护多档回收价（如 128G=350 / 256G=420），列表与详情页均可查看，点「规格✎」编辑
...
- **录入方式**（Web 端「📒 记账」页 / Android APP 首页账本按钮）：
  - 拍照（Android 直接调相机）或选多张图 → 逐张识别图中手机（复用视觉识别 + 报价库匹配，1 图多台、多批上传均可）→ 自动带出型号/品牌/报价库参考价；识别前可填**识别提示词**引导模型；默认收货渠道「路边收」→ **逐台核对：勾选要入账的、✕ 移除不要的、🔄 单台重识别**，再批量入账（只入账勾选的，未勾选保留）
  - 手动录入：输入型号自动联想报价库（带出品牌/分类/参考收价），填收价/渠道/出货价/出货渠道/状态（在库·已出）/日期
- **每台记录字段**：照片、品牌、分类、型号、收购价、渠道（收货）、出货渠道、出货价（可后填）、状态、日期、备注
- **修正**：识别错误时两端均可编辑改型号/价格/渠道/出货渠道；也可删除重录；Web 端对已入账记录可直接「重新识别」照片修正型号；Android 待入账清单每台带「🔄 重识别」（用原图+该机位置框裁剪单独重识别）；明细卡显示预计毛利（出货价−收购价）；识别历史详情里点某台机器可直接进报价库修改该机型价格/备注/型号代码并补图
- **图片**：所有添加图片入口（机型详情补图 / 新增机型 / 识别历史详情补图）均支持**相机临时拍照**或**相册多选**；识别未收录的型号可一键**收录进报价库**并直接拍照补充该机型图片；新增机型资料大多可空（仅型号必填）
- **识别历史管理**：列表页每条可**删除**（确认后连同图片清除），详情页右上角也可删整条；**重识别某台机器成功后自动删除上一次批量识别的错误旧历史**，不留过时记录；历史里未收录的机器点击即可**收录进报价库并补资料**，收录后该历史项自动变为已匹配并重算总价
